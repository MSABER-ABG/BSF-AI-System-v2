...
def render():
    ...
    fig_fi = go.Figure(go.Bar(
        ...
        text=[f"{f['importance']:.1f}%" for f in top_feats], textposition='outside',
    ))
    ...
    with st.expander("⚙️ Customer Profile", expanded=True):
        ...
        with r3:
            cs_segment = st.selectbox("Customer Group", SEGMENT_ORDER, index=2, key="cs_segment")
    ...
    recs = nba_engine_ml({
        ...
        'num_products_owned':len(cs_owned),'engagement_score':25.0,
        'behavior_score':50.0,'app_logins':20,'total_spend_12m':cs_income*10,
        'has_credit_card':int(any('CC_' in p for p in cs_owned)),
        'has_loan':int(any('PL_' in p for p in cs_owned)),
        'late_payment_count':0,'preferred_channel':'Mobile App',
    }, top_n=3)

    if not recs:
        st.info("This customer already owns all available products.")
    else:
        st.markdown(f'<div style="font-size:12px;font-weight:700;color:{ABG_DARK};margin:14px 0 10px;">Top Recommended Products:</div>', unsafe_allow_html=True)
